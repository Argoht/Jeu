...
const XP_RANK_MULTIPLIERS: Array[float] = [
	1.96, # F: environ 3 jours
	1.88, # E: environ 7 jours
	2.98, # D: environ 13 jours
	4.17, # C: environ 20 jours
	6.82, # B: environ 30 jours
	11.91, # A: environ 45 jours
	21.97, # S: environ 60 jours
]
...
func update_derived_stats() -> void:
	var was_hp_full := hp >= max_hp
	var was_stamina_full := stamina >= max_stamina
	max_hp = maxi(1, get_final_stat(StatTypes.KEY_HP))
	max_stamina = maxi(1, get_final_stat(StatTypes.KEY_STAMINA))
	hp = max_hp if was_hp_full else clampi(hp, 0, max_hp)
	stamina = max_stamina if was_stamina_full else clampi(stamina, 0, max_stamina)
	...
	spd = 96 + (get_final_stat(StatTypes.KEY_AGI) * 5) + int(float(get_final_stat(StatTypes.KEY_STAMINA)) * 0.04)
	crit = clampi(5 + int(float(get_final_stat(StatTypes.KEY_AGI)) * 0.7) + int(float(get_final_stat(StatTypes.KEY_WIL)) * 0.25), 5, 45)
...
func get_xp_for_level(l: int) -> int:
	...
	var base_xp := int(XP_TABLE[clampi(l - 1, 0, 98)])
	var multiplier := XP_RANK_MULTIPLIERS[get_rank_index(l)]
	return _round_xp_value(int(round(float(base_xp) * multiplier)))
